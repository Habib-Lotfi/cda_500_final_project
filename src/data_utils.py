...
def load_and_process_citibike_data(year: Optional[int] = None, months_back: int = 13) -> pd.DataFrame:
    ...
    # Step 1: Calculate how far back to go
    today = datetime.today()
    if year:
        months_back = (today.year - year) * 12 + today.month
    ...
    for i in range(months_back):
        ...
        if df_processed is not None:
            ...
            save_path = processed_folder / f"rides_{year_i}_{month:02}.parquet"
...
def transform_raw_data_into_ts_data(rides: pd.DataFrame) -> pd.DataFrame:
    ...
    df = rides.copy(deep=True)
    
    # Reset index to ensure we don't have any index-related issues
    df = df.reset_index(drop=True)
    
    # Check for duplicate column names
    if len(df.columns) != len(set(df.columns)):
        duplicate_cols = set([col for col in df.columns if list(df.columns).count(col) > 1])
        raise ValueError(f"Duplicate columns detected: {duplicate_cols}. Please fix column names before proceeding.")
    ...
    if "pickup_location_id" not in df.columns:
        ...
        station_id_col = "start_station_id" if "start_station_id" in df.columns else "pickup_location_id"
        
        if station_id_col not in df.columns:
            raise ValueError(f"Station ID column '{station_id_col}' not found in DataFrame.")
        
        df = df.dropna(subset=["pickup_datetime", station_id_col])
        df[station_id_col] = df[station_id_col].astype(str)
        df = df[df[station_id_col].isin(TOP_STATIONS)]
        df = df.rename(columns={station_id_col: "pickup_location_id"})
    else:
        # Ensure the timestamp column is named correctly
        if "started_at" in df.columns and "pickup_datetime" not in df.columns:
            df = df.rename(columns={"started_at": "pickup_datetime"})

    # Now perform datetime conversion more carefully
    if "pickup_datetime" not in df.columns:
        raise ValueError("Missing 'pickup_datetime' column after processing.")
    
    # Convert to datetime using a fresh Series to avoid any index issues
    datetime_series = pd.Series(df["pickup_datetime"].values)
    df["pickup_hour"] = pd.to_datetime(datetime_series, errors="coerce").dt.floor("h")
    
    # Drop rows where datetime conversion failed
    df = df.dropna(subset=["pickup_hour", "pickup_location_id"])
